    ...
    if (segment) {
        ...
        state.fetching = internals.fetchFrom(reader, seq, segment, (err, object) => {
            ...
            if (object) {
                ...
                if (object.stream) {
                    reader.watch.set(seq, object.stream);
                    Oncemore(object.stream).once('end', 'error', () => {

                        reader.watch.delete(seq);
                    });
                }
...
exports = module.exports = class HlsSegmentReader extends Readable {
    ...
    constructor(src, options) {
        ...
        this.watch = new Map(); // used to stop buffering on expired segments
        this.fetcher = new SegmentFetcher({ probe: !this.withData, cookie: this.cookie });
        ...
        const updateindex = () => {
            ...
            const stream = UriStream(Url.format(this.url), { headers, timeout: 30 * 1000 });
            stream.on('meta', (meta) => {
